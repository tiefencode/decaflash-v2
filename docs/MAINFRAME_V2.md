...
### Iris-VU auf Basis von Commit `0501020`

Der aktuelle Mainframe enthält Voice-Base-Capture, BeatAnalyzer, AudioFollower und den Sprite-basierten EyeRenderer. Die Iris nutzt jetzt den bereits DC-bereinigten mittleren Absolutpegel jedes 256-Sample-Blocks (16 ms), unabhängig vom Music-Gate. Eine eigene Display-Hüllkurve hat sofortigen Attack und maximal etwa 160 ms Release. Die vorläufigen absoluten Schwellen sind 80 (schwarz) und 2400 (Vollaussteuerung); Codec/Gain und Raumgeräusch müssen am Gerät geprüft werden. Bei fehlenden Audioblöcken wird das VU nach 250 ms schwarz, bei fehlgeschlagenem Capture sofort.

Nach dem ersten Hardwaretest (VU funktioniert laut Nutzer, Darstellung zu grob) besteht die Iris aus 36 unabhängigen, den vollen Iris-Kreis füllenden VU-Polygonen. Wie beim alten Brain-Meter bestimmt eine gemischte Reihenfolge die aktiven Flächen; alle 220 ms wechseln vier aktive/inaktive Plätze. Jede Fläche blendet separat auf/ab und durchläuft Schwarz → Dunkelblau `(8,34,110)` → Hellblau `(40,212,255)` → Magenta `(255,30,230)`. Auch bei Stille bleibt der vollständige Kreis in einem sichtbaren dunklen Blau. Der Audiopegel bestimmt für alle Flächen dieselbe Grundfarbe. Eine deterministische Helligkeitsabweichung von höchstens 25 Prozent wird erst danach auf jeden RGB-Kanal angewandt: Sie erzeugt facettierte hellere und dunklere Varianten derselben Grundfarbe, ohne die Farbskala pro Polygon zu verschieben. Der Augapfel besteht aus 32 großen, flach schattierten Flächen und hat eine harte Lichtquelle oben rechts. Für den günstigen 3D-Blickeffekt werden beim Start 45 Low-Poly-Sclera-Ansichten in PSRAM vorbereitet. Die Irisbewegung wählt nur die nächstgelegene dieser feineren Ansichten; es gibt keine Polygon- oder Trigonometriearbeit im Frame-Loop für den Augenkörper. Die Pupille pulsiert bis zu fünf Pixel, die ganze Iris bis zu zwei Pixel. Der Blick besteht aus kurzen, s-förmig beschleunigten Sakkaden zu zufällig gewählten Zielen und anschließenden Fixationen; er kreist nicht dauerhaft. Bei hoher `attention` werden die Ziele weiter, die Sakkaden etwas kürzer und die Fixationen häufiger. Ab 90 `annoyance` legt sich ein schwarzes, mittig schweres Low-Poly-Oberlid über die Kugel und die Iris wechselt auf ein gesättigtes Dunkelrot → Rot → rotes Orangerot; dieselbe Grenze löst auch das Node-Lichtprofil aus. Ab 90 `loneliness` entsteht ein schwarzes Oberlid, das mittig spitz nach oben zuläuft, und der Blick sinkt leicht ab. Dadurch bewegt sich der vorbereitete weiße Augenkörper zusammen mit der Iris, ohne weitere Last pro Frame. Der Renderer verwendet die vorbereiteten Sclera-Sprites und den bestehenden 128×128-Ausgabe-Sprite mit 25-ms-Frameabstand. Die VU-Auswertung benötigt nur konstante Ganzzahlarbeit pro Audioblock; die 64-ms-BPM-Analyse, Audio-Follow und Funk-/Clock-Logik bleiben unverändert.
...
### B.6 Lokale Stimmungsparameter und Debug-Overlay – Entscheidung vom 26.09.2026

Der Nutzer hat den ersten Schritt konkretisiert: **keine Reaktionstexte**, zunächst nur fünf lokale Parameter; spätere Wirkung auf Nodes ist ein Folgeschritt. Der fehlende Präsenzsensor bleibt ausgeklammert. Langes Drücken des Frontbuttons zeigt die fünf Parameter als Debug-Overlay, erneutes langes Drücken blendet es aus. Dieser Abschnitt ersetzt den vorherigen Entwurf einschließlich Textauswahl. Nachfolgende Nutzerpräzisierung: loneliness steigt ständig und sinkt durch Beschäftigung; Tippen/Mehrfachtippen, Stoß, Drehen/Schiefhalten und Schütteln müssen als getrennte Ereignisse verfügbar sein. Der fehlende Präsenzsensor verhindert diese ausdrücklich gewünschte Interaktionsregel nicht.

Implementiert in `personality.{h,cpp}` und `motion_events.{h,cpp}`, angebunden in `main.cpp`: hardwareunabhängiges, deterministisches Modell mit Werten 0–100, intern Millipunkte. Keine Persistenz, keine dynamische Allokation, keine eigene Task und kein Sprachmodell. `depression` bleibt der humorvolle Grufti-Party-State des Geräts.

| Parameter | Neustart | Vorläufige lokale Regel |
| --- | --- | --- |
| energy | 30 | BPM ist der Hauptfaktor: Nach dem AudioFollower-Lock verwendet Energy dieselbe BPM wie der sichtbare Show-Dot; davor die erkannte BPM. Die Basis liegt bei etwa 13/26/40/60/90 für 80/100/120/140/160 BPM. Ein aktueller, sicherer Beat addiert abhängig von der Confidence nur 0–10. Nach niedrigerem Tempo fällt Energy mit 20 Punkten/s, beim Anstieg mit 4 Punkten/s. Stille senkt den Wert nach 1 s mit 20 Punkten/s bis 0; unbekannte Audioeingänge frieren den Wert ein. |
| annoyance | 0 | Ereignisimpulse siehe unten; auf Nutzerwunsch verlangsamt: Abbau 0,1/s Richtung 0 (ein Punkt je zehn Sekunden). |
| attention | 10 | Ereignisimpulse siehe unten; Rückkehr mit 2/s Richtung 10. |
| loneliness | 100 | Ständig +0,1/s bis 100; Beschäftigung baut den Wert ab, siehe Ereignistabelle. Ein Interaktionszustand, keine gemessene Abwesenheit. |
| depression | 10 | Ziel = 10 + bis zu 50 für langsames Tempo + bis zu 15 für wenig Bass + 15 % von loneliness. Annäherung 0,2 Punkte/s; Beschäftigung −3. |

Alle Werte sind auf 0–100 begrenzt. Die numerischen Zuordnungen sind vorläufige Kalibrierwerte, keine vom Nutzer einzeln bestätigten Charaktereigenschaften. Der State-Service läuft etwa alle 100 ms und zusätzlich vor einem Bewegungsereignis. Lücken über 1 s werden auf 1 s Integration begrenzt und setzen die Audiobestätigung zurück; keine lange nachgeholte Stimmungsentwicklung nach einer blockierten Loop. Zeitdifferenzen funktionieren über `millis()`-Überlauf hinweg.

**Audio – präzisierte Regel:** Lautstärke treibt energy nicht hoch. Vor einem AudioFollower-Lock ist `detectedBpm()` des BeatAnalyzers der Eingang. Nach dem Lock übernimmt Energy `currentBpm`, dieselbe geglättete BPM wie der sichtbare Show-Dot. Die Basis liegt rund bei 13 bis 90 zwischen 80 und 160 BPM: 120 BPM liegen bei 40, 140 bei 60, 150 bei 75 und 160 bei 90. Ein aktueller, sicherer Beat addiert abhängig von der Confidence nur 0 bis 10. Damit erreicht ein 160-BPM-Track mit maximal klaren Beats 100. Halb-/Doppeltempo bleibt eine Grenze des bestehenden Schätzers.

Stille wird unabhängig vom adaptiven Music-Gate am RMS der hochpassgefilterten PCM-Blöcke geprüft: Eintritt unter 80, Austritt ab 120 (16-Bit-Sampleeinheiten). Nach einer Sekunde kontinuierlicher Stille sinkt energy mit 20 Punkten/s bis 0; Rückkehr eines Signals beendet diesen Abfall unmittelbar. Capture nicht bereit bzw. über 250 ms ohne verarbeiteten Block bedeutet Unknown und friert die Audio-Energiewirkung ein; es gilt niemals als Stille. Diese absoluten Schwellen sind Hardware-Kalibrierwerte, keine bereits bestätigten Raumgeräuschgrenzen.

`audio_mood_features.{h,cpp}` analysiert die vorhandenen rohen 16-kHz-PCM-Blöcke zusätzlich mit einem einfachen Hochpass nahe 40 Hz und zwei Tiefpassstufen nahe 180 Hz. Verhältnis von Bassbandleistung zu Gesamtleistung hinter dem Hochpass, ohne FFT, zusätzliche Task oder Audiopuffer. Es ist ein breit auslaufender Bandindikator, keine exakte spektrale Integration von 40–180 Hz. Leistungen werden mit etwa 15 s Zeitkonstante geglättet; der Basswert wird erst nach 15 s brauchbaren Samples freigegeben. Stille, >1 % geclippte Samples pro Block oder Capturelücken verwerfen die Basshistorie. Vorhandene DC-/VU-/BeatAnalyzer-Pfade und Bibliotheksversionen bleiben erhalten.

Depression-Ziel: Basis 10; langsames verlässliches Tempo addiert linear 50 bei ≤80 BPM bis 0 bei ≥160 BPM. Wenig gültiger Bass addiert linear 15 bei ≤8 % Leistungsanteil bis 0 bei ≥25 %. Loneliness addiert unabhängig von Musik maximal 15. Ergebnis maximal 90, langsame Annäherung mit 0,2 Punkten/s. Unbekannte Audioeingänge und Stille liefern keinen Bassmangel- oder Langsamkeitsbeitrag. Audio verändert loneliness nie. Die Bassgewichte und Schwellen müssen mit der Voice Base und realem Musikmaterial geprüft werden.

**Bewegung/Ausrichtung:** Interne IMU aktiviert; maximal alle 20 ms frische Beschleunigungs- und Gyroskopdaten. Der eigenständige hardwareunabhängige Klassifikator verarbeitet 3D-Vektoren statt nur deren Beträge. Die erste ruhige Messung (0,85–1,15 g, <15 Grad/s) definiert die Referenzlage. Ein Tiefpass mit 250-ms-Zeitkonstante schätzt die Schwerkraftrichtung; die Differenz zum aktuellen Sample liefert dynamische Beschleunigung. Die folgenden Grenzen sind ausdrücklich **vorläufige Heuristiken**, keine auf dem Gehäuse validierte Gestenerkennung.

- Beobachtungsfenster 700 ms ab dynamischer Beschleunigung ≥0,20 g oder Drehgeschwindigkeit ≥45 Grad/s. Pro Fenster höchstens ein Ereignis, Priorität Shake → Impact → Rotate → MultiTap → Tap → Move. Dadurch wird ein Schütteln nicht gleichzeitig als mehrere Stöße verrechnet. Gesten an Fenstergrenzen können getrennt werden; Erkennungslatenz bis ungefähr 700 ms.
- **Tap / MultiTap:** kurzer Impuls ab 0,30 g, Ende unter 0,15 g, Dauer maximal 120 ms; Beginn aufeinanderfolgender Impulse mindestens 100 ms auseinander. Zwei oder mehr abgeschlossene Impulse im Fenster ergeben MultiTap mit Zähler. Ein einzelner Impuls gilt nur bei insgesamt höchstens 120 ms Bewegung im Fenster als Tap.
- **Impact:** dynamischer Spitzenwert ≥1,2 g, sofern keine Schüttelsequenz erkannt wurde.
- **Shake:** mindestens drei Richtungswechsel kräftiger Beschleunigungsvektoren (Betrag ≥0,55 g, Skalarprodukt mit voriger Richtung <−0,15 g²). Hoher Pegel allein genügt nicht.
- **Rotate:** mindestens 180 ms im Fenster mit Drehrate ≥45 Grad/s. Erfasst auch Drehen um die Schwerkraftachse.
- **Tilt:** gefilterte Schwerkraftrichtung mehr als 25 Grad gegenüber der Startlage, für mindestens 500 ms ruhig gehalten (Betrag nahe 1 g, Drehrate <15 Grad/s, dynamische Beschleunigung <0,15 g). Rückkehr unter 15 Grad löst den Neigungszustand. Während des weiteren Schiefhaltens alle 2 s ein Tilt-Ereignis; damit baut auch fortgesetztes Halten loneliness ab. Tilt wird außerhalb eines aktiven Bewegungsfensters gemeldet.
- **Move:** mindestens 120 ms allgemeine Bewegung im Fenster ohne spezifischere Klasse. Stillstand in der Referenzlage erzeugt keine Beschäftigung.
- Ungültige Werte oder Samplelücken >150 ms verwerfen angefangene Fenster. Keine nachgeholten Gesten aus fehlenden Daten. Zeitstempel unterstützen `millis()`-Überlauf. Absolute magnetische Himmelsrichtung ist kein Eingang; Drehung und Neigung sind relative Größen aus Gyroskop/Beschleunigung.

| Ereignis | attention | annoyance | loneliness |
| --- | ---: | ---: | ---: |
| Move | +8 | 0 | −5 |
| Tap | +12 | 0 | −8 |
| MultiTap | +18 | +6 | −12 |
| Impact | +20 | +15 | −10 |
| Rotate | +10 | 0 | −6 |
| Tilt, auch weiter gehalten | +5 | 0 | −2 |
| Shake | +25 | +12 | −15 |
...
**Node-Lichtlayer:** Die Stimmungswerte bleiben ausschließlich im Mainframe. Er leitet daraus nach jeder Personality-Aktualisierung eine neutrale `NodeVisualState`-Nachricht ab und wiederholt sie alle drei Sekunden. Sie beschreibt nur die Ausgabe: RGB-Szenen- oder Vollfarbmodus, Helligkeit, eine künftige Farbüberlagerung und das Flash-Verhalten. Der Node speichert die Nachricht zunächst im Empfangs-Callback und setzt diese Render-Vorgaben anschließend außerhalb des Callbacks um. Er kennt dabei weder emotionale Namen noch Schwellenwerte. V1 kennt den zusätzlichen Nachrichtentyp nicht und verwirft ihn.

Die aktuelle Mainframe-Regel ist: bei annoyance ≥90 werden Vollfarbe Rot bei 100 % und Flash dauerhaft an gesendet; ansonsten werden bei loneliness ≥90 Szenenhelligkeit 15 % und Flash aus gesendet; sonst die normalen Szenenwerte. Annoyance hat bei Überschneidung Vorrang. Szene, Clock und Rendererzustand laufen unter den Render-Vorgaben weiter. Wenn der Mainframe wieder die normalen Werte sendet, setzt der nächste Rendererframe die aktuelle Szene sichtbar fort. NodeText und reguläre Flashimpulse sind während einer visuellen Vorgabe unterdrückt. Der Flash-Treiber hat keine echte Dimmstufe; Ausschalten ist die einfache Vorgabe. Farbüberlagerungen werden später als Farbe und Deckkraft aus dem Mainframe gesendet; der Node wendet sie lediglich auf seine gefilterte Szene an.

Hosttests für Personality, Motion, Audio, Beat-/Clock-Follow, Iris und Protokoll einschließlich NodeVisualState bestehen. Mainframe und Node bauen mit den festgesetzten Abhängigkeiten. Der Mainframe wurde am 26.09.2026 auf den angeschlossenen AtomS3R geflasht; jeder Node benötigt die neue Node-Firmware, bevor er die visuellen Vorgaben empfangen kann. Ein Hardwaretest mit einem geflashten V2-Node steht aus. Kein Commit.

Energy verwendet eine BPM-Basis von rund 12–62 mit 140 BPM bei 50 und einen aktuellen Beat-Bonus 0–10; das Tempo bleibt bis zu 15 s aktiv und Stille sinkt bis 0. Kein Commit.

**Button/Display:** Halteschwelle 1200 ms, einmaliges Umschalten während des Haltens. Kurzer Klick startet die Show bzw. wechselt die Szene beim Loslassen. Loslassen nach Langdruck löst keinen Klick aus. Der Button selbst verändert keine Stimmungswerte; physische Bewegung beim Bedienen kann weiterhin die IMU anregen. EyeRenderer zeichnet das Overlay mit fünf beschrifteten Zahlen und Balken im bestehenden 128×128-Canvas vor dem einzelnen Display-Push. Es verwendet den bisherigen 33-ms-Frameabstand und verdeckt einen Teil des laufenden Auges. Zusätzlich zeigt die Debug-Anzeige den letzten Ereignistyp und die Anzahl kurzer Impulse, damit die Unterscheidung am Gerät geprüft werden kann. Keine Personality-Reaktionstexte, Speaker-Ausgabe oder NodeText/Morse-Nachrichten.

**Prüfung:** `sh tests/run_personality.sh` prüft BPM-Ziele/Confidence, unabhängige loneliness, schnelle Stille/Reaktion auf Audio-Rückkehr und unbekannte Inputs sowie Depressionseinflüsse. `sh tests/run_audio_mood_features.sh` prüft synthetische 100-/1000-Hz-Signale, Lautstärkeunabhängigkeit des Bassverhältnisses, Aufwärmzeit, Stille, DC, Capturelücken, Clipping und Zeitüberlauf. `sh tests/run_motion_events.sh` prüft synthetische Spuren für Tap/MultiTap, Impact, Rotate, gehaltenen Tilt, Shake, Stillstand, Datenlücken, ungültige Samples und Zeitüberlauf. Dazu Protokoll-, BeatAnalyzer-, AudioFollower- und Iris-VU-Hosttests sowie `pio run -e mainframe`. Hardwareabnahme steht aus: dieselben Gesten mehrfach ausführen, letzte Ereignisklasse im Overlay vergleichen, Tippstärke/Montage/Bassvibrationen prüfen, Neigungsreferenz beim Start beachten, Audio-/Clock-Kontinuität unter IMU- und Displaylast messen. Synthetische Tests belegen die Regeln, nicht deren Trennschärfe am Gehäuse. Am 26.09.2026 auf ausdrücklichen Nutzerauftrag auf den AtomS3R an `/dev/cu.usbmodem101` geflasht (ESP32-S3-PICO-1, MAC `14:c1:9f:d5:6e:ec`): Upload erfolgreich, Flash-Hash verifiziert, Hardware-Reset ausgelöst. Die anschließende BPM-/Bass-/Stille-Version wurde auf demselben Gerät ebenfalls erfolgreich geflasht, Hash verifiziert und Reset ausgelöst (Flash 917.901 Byte, statisches RAM 55.164 Byte laut Build). Alle sieben Hosttests bestehen. Der anschließend auf 0,1 Punkt/s verlangsamte Annoyance-Abbau wurde mit Personality-/Protokolltests und Mainframe-Build geprüft und ebenfalls erfolgreich geflasht (Hash verifiziert, Reset ausgelöst). Energy kann bei auslaufender Tempo-Evidenz auf dem erreichten Wert stehen bleiben; die vom Nutzer gemeldete dauerhaft hohe Energy ist ohne aktuelle Eingangs-Messwerte noch nicht einer Ursache zugeordnet. Bass-/Stilleschwellen, Erkennungsqualität und Laufzeit unter realer Musiklast bleiben am Gerät zu prüfen. Kein Commit.
...
1. Gegenseitige Ablehnung aller fünf V2-Pakettypen zwischen V1 und V2 bei weiterhin gültiger V2-Kommunikation; anschließend Offline-Start aller Node-Typen/Rollen; unterschiedliche Bootreihenfolge; Node-/Mainframe-Neustart; Rollenwechsel; mehrfacher Szenenwechsel und gezielte Paketverluste.
